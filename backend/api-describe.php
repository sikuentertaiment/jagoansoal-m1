...
if (empty($material)) {
    echo json_encode(['error' => 'Material content is required']);
    exit();
}
...
try {
    ...
    $cost = max(3, (int)ceil($totalQuestions / 5));

    if (!$user || $user['credit'] < $cost) {
        echo json_encode(['error' => 'insufficient_credits', 'message' => "Minimum {$cost} credits required for material-based generation"]);
        exit();
    }
    ...
    if ($type === 'mixed') {
        ...
        $prompt = "Berdasarkan materi berikut:\n\n{$material}\n\nBuatkan {$totalQuestions} soal Campuran untuk mata pelajaran {$subject} kelas {$class} dengan tingkat kesulitan {$difficultyLabel} tentang topik \"{$topic}\". ";
        ...
        $prompt = "Berdasarkan materi berikut:\n\n{$material}\n\nBuatkan {$totalQuestions} soal {$typeLabel} untuk mata pelajaran {$subject} kelas {$class} dengan tingkat kesulitan {$difficultyLabel} tentang topik \"{$topic}\".";
        if (!empty($extraInstructions)) {
            $prompt .= " Catatan tambahan: {$extraInstructions}";
        }
    ...
    $stmt->execute([$userId, -$cost, 'Generate soal dari materi ' . $totalQuestions . ' soal: ' . $topic]);
    ...
    error_log('API describe generate error: ' . $e->getMessage());
